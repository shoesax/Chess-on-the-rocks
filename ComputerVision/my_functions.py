...
def segment_the_image(img, g_tolerance):
    ...
    for i in range(8):
        ...
        for j in range(8):
            ...
            if j > 3:
                if j == 7:
                    squares[square_number] = colum[int(last_line_location) + 8: int(line_location) + tolerance + 10,
                                             0:width]
                else:
                    squares[square_number] = colum[int(last_line_location) + 8: int(line_location) + tolerance + 2,
                                             0:width]
            else:
                squares[square_number] = colum[int(last_line_location) + tolerance: int(line_location) + tolerance,
                                         0:width]
            ...
            if np.sum(squares[square_number]) == 0:
                continue
            cv.imwrite(file_path + "/suqare_" + str(coun) + "_" + str(i) + "_" + str(j) + ".PNG",
